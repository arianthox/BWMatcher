...
import com.arianthox.predictor.hmm.model.HMMModel;
...
public class HiddenMarkov {
    ...
    public void train() {
        ...
        for (int i = 0; i < 20; i++) {
            reestimate();
            log.log(Level.FINE,"reestimating.....");
        }
        //
        // oldm=
    }
...
//    }
    public HiddenMarkov( Path path ) throws Exception{
        ObjectIO<HMMModel> in = new ObjectIO<>();
        HMMModel model = in.readModel(path);
        num_obSeq = model.getNum_obSeq( );
        output = model.getOutput( );// ArrayWriter.print2DTabbedDoubleArrayToConole(output);
        transition = model.getTransition( );
        pi = model.getPi( );
...
//    }
    ...
    public void save(Path path) throws Exception {
       // PatternFileIO<HMMModel> oio = new PatternFileIO<>();
        HMMModel model = new HMMModel();
        model.setOutput(output);
        //ArrayWriter.print2DTabbedDoubleArrayToConsole(output);
        model.setPi(pi);
